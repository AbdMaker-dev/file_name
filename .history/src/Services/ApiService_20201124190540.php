...
class ApiService
{
    ...
    public function __construct(CompetenceRepository $repo_Competence,ProfileRepository $profile, ProfileRepository $repo_profil, SerializerInterface $serializer, ValidatorInterface $validator, UserPasswordEncoderInterface $encoder)
    {
    ...
    public function creatApprenant($tabEmails)
    {
        ...
        foreach ($tabEmails as $key => $mail) {
            ...
            $new_appre->setProfile($this->repo_profil->findOneByLibelle("APPRENAN"));
            $new_appre->hydrate();
