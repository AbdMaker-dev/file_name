...
class ApiService
{
    ...
    public function assigned($object ,$attre, $tabVal, $repo)
    {
        $add_attre = "add".ucfirst($attre);
        $find_attre = "findOneBy".ucfirst($attre);
        foreach ($tabVal as $value) {
            
            $object->$add_attre($repo->$find_attre($validator))
        }
    }
}