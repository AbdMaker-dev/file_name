...
class UserFilter extends AbstractContextAwareFilter
{
    ...
    $value= ["formateur", "cm"];
        ...
        // dd($classMetadata->discriminatorMap); App\Entity\User" et ses fille
        ...
        foreach ($classMetadata->discriminatorMap as $discriminator => $entityClass) {
            ...
            // dd($metadata->getShortName()); nom des class
        ...
        $dt = $queryBuilder->select('*')
        ->from('user', 'u')
        ->where('u.type != apprenant')
        ->setParameter("instance","user");
