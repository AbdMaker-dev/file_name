...
class BriefController extends AbstractController
{
     ...
     */
    public function postBriefs(Request $request, BriefRepository $repo_brief, ApiService $apiService, SerializerInterface $serializer, EntityManagerInterface $em, NiveauRepository $repo_niv, TagsRepository $repo_tag, FormateurRepository $repo_forma, EtatBriefRepository $repo_etaBrief): Response
    {
        ...
        $ressource = $request->files->get("pieceJointe");
        if ($ressource){
            $ressource = fopen($ressource->getRealPath(), "rb");
            $data["ressource"]["pieceJointe"] = $ressource;
        }
        ...
        foreach ($request->files as $key => $file) {
            if($key=="ressource"){
                foreach ($file as $key => $value) {
                    $ress = $value->get("pieceJointe")
                    $data["ressource"]["pieceJointe"] = ;
                }
            }else{
                $data["image"] = $file->get("image");
            }
        }

        $data = $apiService->unsetter($data, ["niveauCompetences", "tags", "formateur", "etatBrief"]);
