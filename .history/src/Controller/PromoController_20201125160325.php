...
class PromoController extends AbstractController
{
     ...
     */
    public function updatePromoIdAppre(int $id, Request $request, PromoRepository $repo_promo,ApprenantRepository $repo_appre, ApiService $apiService): Response
    {
        $data = json_decode($request->getContent(), true);
        if ($data==null) {
        ...
        $Apprenants_add = $apiService->creatApprenant($data["add"],$request->files->get("fichier"));
        $promo->setApprenants($Apprenants_add);

        dd($data["add"]);

        foreach ($data['remove'] as $key => $emailAppre) {
            $appre = $repo_appre->findOneByEmail()
        }
