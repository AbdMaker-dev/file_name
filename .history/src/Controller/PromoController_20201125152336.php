...
class PromoController extends AbstractController
{
    ...
    public function addPromo(Request $request, SerializerInterface $serializer, ApiService $apiService, EntityManagerInterface $em): Response
    {
        ...
        // gestion apprenants
        $new_promo->setApprenants$apiService->creatApprenant($request->request->all()["apprenants"],$request->files->get("fichier"))(     $apiService->creatApprenant($data[1]['apprenants']));
     ...
     */
    public function updatePromoIdAppre(int $id, Request $request, PromoRepository $repo_promo, ApiService $apiService): Response
    {
        ...
        //     dd($array);
        // }
        // dd($request->files->get("fichier"));

        $emailApprenat = $apiService->creatApprenant($request->request->all()["apprenants"],$request->files->get("fichier"));
        dd($emailApprenat);
        return $this->json([
            'message' => 'Welcome to your new controller!',
            'path' => 'src/Controller/PromoController.php',
