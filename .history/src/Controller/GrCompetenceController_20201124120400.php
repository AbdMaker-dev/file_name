...
class GrCompetenceController extends AbstractController
{
     ...
     */
    public function addGroupeComptence(Request $request, SerializerInterface $serialize, GroupeCompetenceRepository $repo_grC): Response
    {
        ...
        foreach ($data_gr_competence['competences'] as $key => $comptence) {
            if (isset($comptence['id'])){
                $new_gr_competence->addCompetence($repo_grC->findOneById($comptence['id']));
            }else{
                $new_gr_competence->addCompetence($serialize->denormalize($comptence, "App\Entity\Compte"))
            }
        }
