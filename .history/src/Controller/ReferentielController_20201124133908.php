...
class ReferentielController extends AbstractController
{
     ...
     */
    public function updateReferentiel(int $id, Request $request, ReferentielRepository $repo_ref,CritereAdmissionRepository $repo_cri_add, ApiService $apiService, SerializerInterface $serializer, CompetenceRepository $repo_competence, EntityManagerInterface $em): Response
    {
        // security du route
        if (in_array("ROLE_APPRENANT", $this->getUser()->getRoles())) {
            return $this->json([
        ...
        $new_ref->setCrAdmission($apiService->serialize($cri_admi, "CritereAdmission", new CritereAdmissionRepository()));
        $new_ref->setCrEvaluation($apiService->serialize($cri_eva, "CritereEvaluation"));
