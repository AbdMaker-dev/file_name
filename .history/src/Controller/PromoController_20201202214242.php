...
class PromoController extends AbstractController
{
     ...
     */
    public function getPromoIdGroupId(int $id, int $di, GroupeRepository $repo_gr, SerializerInterface $serializer): Response
    {
        $reslt = $repo_gr->findOneByIdGroupeIdPromo($di, $id);

        if (!$reslt) {
            return $this->json(['message' => 'not fond', 'code' => 404,]);
        }

        $reslt = $serializer->serialize($reslt, "json", ["groups"=>"groupe:read"]);

        return $this->json(['message' => 'Oki', 'code' => 200, 'result'=> $reslt]);
    }

    /**
     * @Route("/promo/{id}/groupes/{di}/apprenants", name="get_promo_id_group_id", methods={"GET"})
     */
    public function getPromoIdGroupIdApprenants(int $id, int $di, GroupeRepository $repo_gr, SerializerInterface $serializer): Response
    {
        ...
        return $this->json(['message' => 'Okii', 'code' => 200, 'result'=> $reslt]);
    }
